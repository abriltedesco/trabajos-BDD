...
begin
    ...
    set top1 = (SELECT productName FROM products
	JOIN orderdetails ON products.productCode = orderdetails.productCode 
	JOIN orders ON orderdetails.orderNumber = orders.orderNumber WHERE year(orderDate) = anio
	GROUP BY products.productCode ORDER BY sum(quantityOrdered) DESC LIMIT 1);

	set top2 = (SELECT productName FROM products
	JOIN orderdetails ON products.productCode = orderdetails.productCode 
	JOIN orders ON orderdetails.orderNumber = orders.orderNumber WHERE year(orderDate) = anio
	GROUP BY products.productCode ORDER BY sum(quantityOrdered) DESC LIMIT 1 OFFSET 2);

	set top3 = (SELECT productName FROM products
	JOIN orderdetails ON products.productCode = orderdetails.productCode 
	JOIN orders ON orderdetails.orderNumber = orders.orderNumber WHERE year(orderDate) = anio
	GROUP BY products.productCode ORDER BY sum(quantityOrdered) DESC LIMIT 1 offset 3);
...
DROP FUNCTION topVendidos;
	...
	GROUP BY products.productCode ORDER BY cantTotal DESC LIMIT 1;